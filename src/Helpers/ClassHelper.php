...
class ClassHelper
{
    public static function isKindOf($class, string $parentClass): bool {
        return (
            $class === $parentClass
            || is_subclass_of($class, $parentClass)
            || $class instanceof $parentClass
        );
    }
    public static function parseClass(string $classFullname, bool $nullable, bool $onlyFromDocblock, SchemaHelper $schemaHelper)
    {
        if (ClassHelper::isKindOf($classFullname, 'Illuminate\Database\Eloquent\Model')) {
            ...
            foreach ($publicProperties as $property) {
                ...
                foreach ($propertyTypes as $propertyType) {
                    if ($propertyType !== null) {
                        ...
                        } else if (ClassHelper::isKindOf($propertyTypeName, 'DateTimeInterface')) {
